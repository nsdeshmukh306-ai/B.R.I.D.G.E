...
class ValidationResult(BaseModel):
    ...
    def summary(self) -> str:
        return (f"mean={self.mean_error_px:.2f}px max={self.max_error_px:.2f}px "
                f"n={self.n_points} threshold={self.threshold_px:.1f}px -> {'VALID' if self.valid else 'INVALID'}")
...
class CalibrationValidator:
    ...
    def validate(self, H: np.ndarray, camera_pts: np.ndarray, projector_pts: np.ndarray,
        ...
        errs = reprojection_errors(H, camera_pts, projector_pts)
        ratio = float(np.mean(inlier_mask.ravel() > 0)) if inlier_mask is not None else 1.0
        mean, mx, med = float(np.mean(errs)), float(np.max(errs)), float(np.median(errs))
